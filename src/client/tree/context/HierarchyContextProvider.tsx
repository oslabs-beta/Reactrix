...
export function HierarchyContextProvider({ children, onExpandNodes, treeRef, data }: ISidebarDrawerProps): JSX.Element {
    ...
    const nestedObjectToArray = useCallback((data: INestedObject) => {
        ...
        const loopChildren = (dataChildren: INestedObject, parentId?: number | string) => {
            const insert = {
                id: dataChildren.id,
            ...
            array.push(insert);
            if (Array.isArray(dataChildren.children) && dataChildren.children.length > 0)
    ...
    const arrayToNestedObject = useCallback((data: IParsedArray[]): INestedObject => {
        ...
        const loopArray = (dataArray: IParsedArray[], parentId: number | string) => {
    ...
    const editById = useCallback((id: number | string, data: Partial<INestedObject>, action = 'add' as 'replace' | 'add' | 'remove', nestedObject?: INestedObject) => {
        let nestedObjectClone = nestedObject ? { ...nestedObject } : { ...hierarchyRef.current };
        nestedObjectClone = clone(nestedObjectClone);

        if (nestedObjectClone.id === id) {
            if (!action || action === 'replace') return { ...nestedObjectClone, ...data };
            if (!action || action === 'add')
                return {
                    ...nestedObjectClone,
                    ...data,
                    children: [...nestedObjectClone.children, ...(data.children ? data.children : [])]
                };
            if (!action || action === 'remove')
                return {
                    ...nestedObjectClone,
                    ...data,
                    children: [...nestedObjectClone.children.filter((child) => (data.children ? !data.children.map((i) => i.id).includes(child.id) : child))]
                };
        }

        if (!nestedObjectClone.children) return nestedObjectClone as INestedObject;

        const newChildren: INestedObject[] = nestedObjectClone.children.map((child) => {
    ...
    const removeById = useCallback((id: number | string, dataToRemove: Array<number | string>, nestedObject?: INestedObject) => {
        let nestedObjectClone = nestedObject ? { ...nestedObject } : { ...hierarchyRef.current };
        nestedObjectClone = clone(nestedObjectClone);
        ...
        if (nestedObjectClone.id === id) {
            return {
                ...nestedObjectClone,
                ...dataToRemove,
                children: [...nestedObjectClone.children.filter((child) => (dataToRemove ? !dataToRemove.map((i) => i).includes(child.id) : !child))]
            };
        }
    ...
    const findParentByChildId = useCallback((id: number | string, nestsObject?: INestedObject) => {
        let nestedObject = nestsObject ? { ...nestsObject } : { ...hierarchyRef.current };
        nestedObject = clone(nestedObject);

        const loop = (childObject: INestedObject, parentObject: INestedObject | null, arrayParentIdPaths = [] as Array<number | string>) => {
    ...
    useImperativeHandle(
        ...
        [addChildrenById, arrayToNestedObject, editById, findById, findParentByChildId, nestedObjectToArray, onExpandNodes, removeById]
    );
...
export const useHierarchyData = (): IHierarchyContextData => useContext(HierarchyContext);
