...
export const RenderChildren = ({ list, data, prop }: IRenderChildren) => {
  if (Array.isArray(list) && list.length) {
    return (
      <ChildrenComponent id={`children_${data.id}`} className={'org-tree-node-children'} horizontal={!!prop.horizontal} strokeColor={prop.strokeColor} strokeWidth={prop.strokeWidth}>
        {list.map((item) => {
